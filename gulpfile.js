...
function modules() {
  ...
  var jqueryuimage = gulp.src([
    './node_modules/jquery-ui-dist/images/*'
  ])
    .pipe(gulp.dest('./public/vendor/jqueryui/images'));
  ;

  return merge(bootstrapJS, bootstrapSCSS, chartJS, dataTables, fontAwesome, jquery, jqueryui, jqueryuimage, jqueryEasing, toastr, toastrSCSS, sweetalert2, sweetalertSCSS, sweetalert2SCSS, select2, select2SCSS, select2CSS, dropzone, dropzoneCSS, apex, apexCSS, quill, perfectScrollbarCSS, perfectScrollbar, katex);
}
...
function watchFiles() {
  ...
  gulp.watch(["./public/js/**/*", "!./public/js/**/*.min.js"], gulp.series('js'));
  gulp.watch("./**/public/**/*.php", gulp.series('browserSyncReload'));
}
